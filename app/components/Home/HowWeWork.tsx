...
const ArrowSvg = ({ className }: { className?: string }) => (
  <svg className={`text-white opacity-90 drop-shadow-[2px_2px_0_rgba(0,0,0,0.5)] transition-transform duration-700 hover:scale-110 ${className}`} viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round">
...
const ProcessCard = ({ number, title, desc, rotate }: { number: string; title: string; desc: string; rotate: string }) => (
  <div className={`relative w-full max-w-[320px] bg-white/5 backdrop-blur-md border border-white/10 rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] p-8 ${rotate} transition-transform duration-500 hover:-translate-y-3 hover:shadow-[0_25px_60px_rgba(0,0,0,0.4)] z-20 inline-block`}>
    <div className="absolute inset-[6px] border border-white/10 rounded-[26px] pointer-events-none"></div>

    <div className="absolute top-4 left-1/2 -translate-x-1/2 w-5 h-5 bg-white/20 rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] flex items-center justify-center border border-white/20">
      <div className="w-[6px] h-[6px] bg-white rounded-full shadow-sm"></div>
    </div>

    <div className="mt-8 text-left">
      <span className="text-xl text-gray-300 font-semibold">{number}</span>
      <h3 className="text-2xl md:text-[28px] font-bold text-white mt-2 mb-4 tracking-tight">{title}</h3>
    ...
    <section className="bg-transparent py-24 md:py-32 overflow-hidden relative">
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-12 relative z-20">
          ...
          >
            <div className="mb-6">
              <span className="inline-block border border-white/20 rounded-full px-5 py-2 text-sm font-semibold text-white tracking-wide bg-white/10 backdrop-blur-sm">
                How we work
              </span>
            </div>
            
            <h2 className="text-4xl md:text-5xl lg:text-[54px] font-bold leading-[1.15] text-white tracking-tight">
              How We Drive Digital Growth for Your Business
            </h2>
            
            <p className="text-gray-300 text-base md:text-[17px] leading-relaxed mt-6 font-medium max-w-md">
              {introText}
            </p>

            <ArrowSvg className="hidden lg:block absolute bottom-4 -right-12 w-16 h-16 rotate-[45deg] z-50 text-white" />
          </motion.div>
        ...
        <div className="hidden lg:flex w-full justify-center -my-24 z-0 relative pointer-events-none">
          <svg className="w-[800px] h-[320px] opacity-40" viewBox="0 0 800 320" fill="none" stroke="#fff" strokeWidth="2" strokeDasharray="6 6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end gap-10 lg:gap-0 mt-12 lg:mt-0 relative z-20">
          ...
          >
            <div className="flex items-center gap-4 rotate-[2deg]">
              <svg className="hidden lg:block w-24 h-12 text-white/50" viewBox="0 0 100 50" fill="none" stroke="currentColor" strokeWidth="2" strokeDasharray="5 5">
                <path d="M 10 10 Q 50 40 90 30" />
              </svg>
              <span className="text-3xl text-gray-300 tracking-wider font-semibold" style={{ fontFamily: "Caveat, 'Comic Sans MS', cursive" }}>
